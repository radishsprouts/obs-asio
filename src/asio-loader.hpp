...
struct asio_data {
	obs_source_t *source;
	ASIOAudioIODevice *asio_device;           // device class (non null only while the source is a client of it)
...
public:
	...
	/* obs_clients is read by the asio driver thread (processBuffer) and modified by the obs threads which
	 * create / destroy the sources, so every access must hold this mutex. Once removeClient() has returned,
	 * the driver thread no longer references the removed source.
	 */
	std::mutex clients_mutex;

	void addClient(struct asio_data *client)
	{
		std::lock_guard<std::mutex> lock(clients_mutex);
		if (std::find(obs_clients.begin(), obs_clients.end(), client) == obs_clients.end())
			obs_clients.push_back(client);
	}

	void removeClient(struct asio_data *client)
	{
		std::lock_guard<std::mutex> lock(clients_mutex);
		obs_clients.erase(std::remove(obs_clients.begin(), obs_clients.end(), client), obs_clients.end());
	}

	size_t getClientCount()
	{
		std::lock_guard<std::mutex> lock(clients_mutex);
		return obs_clients.size();
	}
...
public:
	...
	~ASIOAudioIODevice()
	{
		/* Stop the driver and release its COM object first, then free the buffers it was using and
		 * uninitialize COM. Freeing the buffers or calling CoUninitialize() while the driver is still
		 * started / its COM object still alive is undefined behaviour.
		 */
		close();
		...
		// CoInitialize() and CoUninitialize() must be balanced on the same thread
		if (bComInitialized && comThreadId == GetCurrentThreadId())
			CoUninitialize();
	...
	void close()
	{
		...
		if (asioObject != nullptr && deviceIsOpen) {
			...
			/* The obs clients are kept on purpose: close() is also used when the driver requests a reset
			 * (timerCallback) and the sources must keep receiving audio once the device is reopened.
			 * Clients are only removed by detach_device(), when a source is destroyed or changes device.
			 */
...
private:
	...
	// initialized so that the destructor can free() them even when openDevice() failed before allocating
		...
		if (loadDriver()) {
			...
			if (errorstring.empty()) {
				...
				if (asioObject != nullptr &&
					...
					/* allocate buffers (openDevice() can run again from open(), don't leak the previous ones) */
					free(bufferInfos);
					free(inputFormat);
					free(outputFormat);
	...
	void processBuffer(long bufferIndex)
	{
		...
		// held while the sources are used, so that asio_destroy() can't free a source in the middle
		std::unique_lock<std::mutex> lock(clients_mutex);
		bool hasClients = !obs_clients.empty();
